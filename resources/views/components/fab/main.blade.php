...
<div class="fab">
    ...
    @if ($settings->phone_active && $settings->phone_number)
        <a href="tel:{{ $settings->phone_number }}" class="btn btn-lg p-5"><svg xmlns="http://www.w3.org/2000/svg"
    ...
    @if ($settings->whatsapp_active && $settings->whatsapp_number)
        <a href="https://wa.me/{{ $settings->whatsapp_number }}?text=👋%20Hello%20Forno,%20I%20would%20like%20to%20place%20an%20order!"
            target="_blank" class="btn btn-lg p-5"><img src="{{ asset('icons/whatsapp-icon.png') }}" alt=""
                class="w-10"></a>
    @endif

    @if ($settings->instagram_url && $settings->instagram_active)
        <a href="{{ $settings->instagram_url }}" class="btn btn-lg p-5"><img src="{{ asset('icons/instagram.png') }}"
                alt="" class="w-10"></a>
    @endif

    @if ($settings->facebook_url && $settings->facebook_active)
        <a href="{{ $settings->facebook_url }}" class="btn btn-lg p-5"><img src="{{ asset('icons/facebook.png') }}"
                alt="" class="w-10"></a>
    @endif

    @if ($settings->location_url && $settings->location_active)
        <a href="{{ $settings->location_url }}" class="btn btn-lg p-5"><img src="{{ asset('icons/google-maps.png') }}"
                alt="" class="w-10"></a>
    @endif
