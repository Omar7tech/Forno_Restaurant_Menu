...
<div class="card w-full bg-base-200/60  {{-- shadow-[2px_0_3px_-1px_rgb(0_0_0_/_0.1)] --}} p-2 rounded-s-full rounded-e-2xl">
    <div class="card-body flex-row items-center p-0">
        <div class="avatar ">
            <div
                class="{{ $product->featured ? 'ring-yellow-500/60' : 'ring-teal-700/60' }} ring-offset-base-100 w-13 rounded-full ring-1 ring-offset-2">
                <img src="https://picsum.photos/200/300" />
            </div>
        </div>
        <div class="flex-1 border-l-2 border-base-300/50 ps-2">
            <div class="card-title text-sm mb-0 flex items-center">
                @if ($product->featured)
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"
                        class="size-4 fill-amber-500">
                ...
                @endif
                @if ($product->new)
                    <img src="{{ asset('icons/new.png') }}" alt="" class="w-5">
                @endif
                @if ($product->name)
            ...
            @if ($product->description)
                <p class="text-xs md:text-sm lg:text-md text-gray-500 mb-0">{{ $product->description }}</p>
            @endif
        </div>
        @if ($product->price)
